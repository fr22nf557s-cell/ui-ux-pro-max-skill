...
## Section plan (one layout family per section, no consecutive repeats)

1. Film chapters (scroll-scrub, six chapters over the one take): family = full-bleed cinematic stage with bottom-left copy.
2. Current instructions: family = filter bar + card grid.
3. Run the numbers: family = two-column calculator cards with stat tiles.
4. Which survey: family = comparison bars + data table beside a detail panel.
5. Speak to an advisor: family = split intro + form.
6. Footer: family = three-part rule-topped strip.

Eyebrow ration: 6 sections -> at most 2 eyebrows (Current instructions, Speak to an advisor). Chapter kickers inside the film stage are the film's own labels, not section eyebrows.

Anti-convergence: previous build in this chat was the illustrated GSAP prototype (SVG rooms, charcoal + gold, Cormorant + Montserrat, pinned zoom, glass cards). This build differs on world/subject (real photoreal manor film), Tier-1 technique (scroll-scrub film), CTA garments (bespoke per-CTA chrome), corner language (pills + 6px slabs), hero architecture (chapters over one take). Palette and type are kept because the client mandated them.
